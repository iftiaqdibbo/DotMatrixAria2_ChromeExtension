...
(function() {
  ...
  const siteInterceptors = [
    { pattern: /(https:\/\/store\d+\.gofile\.io\/download\/[^"'}\s]+)/g, name: 'Gofile' },
    { pattern: /(https:\/\/file-[a-z\-]+\d*\.gofile\.io\/download\/[^"'}\s]+)/g, name: 'Gofile' },
    { pattern: /(https:\/\/1fichier\.com\/[^\s"'}]+)/g, name: '1Fichier' },
    { pattern: /(https:\/\/pixeldrain\.com\/api\/file\/[^\s"'}]+)/g, name: 'Pixeldrain' },
    { pattern: /(https:\/\/mediafire\.com\/file\/[^\s"'}]+)/g, name: 'MediaFire' },
    { pattern: /(https:\/\/download\d*\.mediafire\.com\/[^\s"'}]+)/g, name: 'MediaFire' },
    ...
    { pattern: /(https:\/\/swisstransfer\.com\/[^\s"'}]+)/g, name: 'SwissTransfer' },
    { pattern: /(https:\/\/[a-z0-9]+\.swisstransfer\.com\/[^\s"'}]+)/g, name: 'SwissTransfer' },
    ...
    { pattern: /(https:\/\/downmediaload\.com\/[^\s"'}]+)/g, name: 'DownMediaLoad' },
    ...
    { pattern: /(https:\/\/clicknupload\.org\/[^\s"'}]+)/g, name: 'ClickNUpload' },
    { pattern: /(https:\/\/dailyuploads\.net\/[^\s"'}]+)/g, name: 'DailyUploads' },
  ...
  const jsonSiteInterceptors = [
    { urlPattern: /pixeldrain\.com\/api\/file/, jsonPath: 'download_url', name: 'Pixeldrain' },
  ];
  ...
  function scanResponse(text, responseUrl, source) {
    ...
    for (const match of allMatches) {
      console.log('[Aria2 Content] ' + match.site + ': Found download URL in ' + source + ':', match.url);
  ...
  XMLHttpRequest.prototype.send = function(...args) {
    this.addEventListener('load', function() {
      try {
        ...
        isHijackEnabled().then(function(enabled) {
          if (enabled) {
